...
graph_builder = StateGraph(DataAgentState, DataAgentContext)
...
async def test():
    ...
    async with (
        ...
        column_qdrant_repository = ColumnQdrantRepository(qdrant_client= qdrant_client_manager.client)
        metric_qdrant_repository = MetricQdrantRepository(qdrant_client= qdrant_client_manager.client)
        column_value_es_repository = ColumnValueEsRepository(client= es_client_manager.client)
        meta_mysql_repository = MetaMysqlRepository(session= meta_session)
        dw_mysql_repository = DwMysqlRepository(session=dw_session)


        state = DataAgentState(query="查询李伟买了哪些产品，在哪里买的，总共花了多少钱")
